...
export default function DonationFailedContent() {
  const t = useTranslations('donate.failed')
        ...
        if (response.ok) {
          ...
          if (data.donation) {
            setDonation({
              ...
              failureReason: data.donation.failureReason || reason || t('description'),
      ...
      <div className="container-custom py-20 lg:py-32">
        ...
        >
          ...
          <p className="text-lg text-gray-600 mb-8">
            {donation?.failureReason || reason || t('description')}
          </p>
          ...
          {donation && (
            <div className="bg-white rounded-2xl p-6 mb-8 shadow-lg border border-gray-100">
              <h3 className="font-medium text-gray-900 mb-4">{t('paymentDetails')}</h3>
          ...
          <div className="mt-12 p-6 bg-amber-50 border border-amber-200 rounded-2xl">
            <h3 className="font-medium text-amber-900 mb-2">{t('needHelp.title')}</h3>
          ...
          <div className="mt-8 text-left bg-white rounded-2xl p-6 shadow-lg border border-gray-100">
            <h3 className="font-medium text-gray-900 mb-4">{t('commonIssues.title')}</h3>
            <ul className="space-y-3 text-sm text-gray-600">
              <li className="flex items-start gap-2">
                <span className="text-red-500 mt-0.5">•</span>
                <span>{t('commonIssues.insufficientFunds')}</span>
              </li>
              <li className="flex items-start gap-2">
                <span className="text-red-500 mt-0.5">•</span>
                <span>{t('commonIssues.timeout')}</span>
              </li>
              <li className="flex items-start gap-2">
                <span className="text-red-500 mt-0.5">•</span>
                <span>{t('commonIssues.declined')}</span>
              </li>
              <li className="flex items-start gap-2">
                <span className="text-red-500 mt-0.5">•</span>
                <span>{t('commonIssues.networkError')}</span>
              </li>
            </ul>
          </div>
        </motion.div>
